...
//that have been aligned to a pangenome reference file from roary
// Written by Asher, largely based on CollectGeneAlignments code by
// Mingzhi Lin (mingzhi9@gmail.com)
package main
...
func main() {
	app := kingpin.New("CollectPangenomeAlignments", "Collect gene alignments which have been aligned to pangenome reference from Roary.")
	app.Version("0.4.1")
	sampleFile := app.Arg("list_file", "file contains a list of aligned genomes").String()
	//gffFile := app.Arg("gff_file", "gff file").String()
	pangenomeRef := app.Arg("pangenome_ref", "Pangenome reference genome generated by Roary").String()
	dataDir := app.Arg("data_dir", "folder containing aligned genome sequences").String()
	...
	appendix := app.Flag("appendix", "appendix (default .pileup.fasta)").Default(".pileup.fasta").String()
	...
	//read the pangenome reference to generate genbank-style headers
	headerMap := mapPangenome(*pangenomeRef)
	...
	for id := range geneIDChan {
		alignChan := pullGenes(id, headerMap, jobs, *ncpu)
		numGenes := 0
		alnLength := -1
...
func pullGenes(id string, headerMap map[string]string, jobList []*LoadingJob, ncpu int) chan []seq.Sequence {
	jobChan := make(chan *LoadingJob)
	go func() {
		defer close(jobChan)
		...
		go func() {
			for job := range jobChan {
				...
				for j, gene := range genes {
					...
					genbankId := headerMap[id]
					gene.Id = genbankId + " " + job.samples[j]
					genes[j] = gene
				}
				resChan <- genes
			}
...
func (job *LoadingJob) Load(sampleChan chan string, numGenome int) {
	...
	go func() {
		...
		for sample := range sampleChan {
			genes := readGenome(sample, job.dataDir, job.appendix)
...
//mapPangenome reads the pangenome reference genome, finds the headers
//and returns a keymap which maps the gene ids that show up in the consensus genomes
//to a proper genbank header
func mapPangenome(pangenome string) (headerMap map[string]string) {
	f, err := os.Open(pangenome)
	if err != nil {
		log.Fatalf("Error when reading pangenome reference genome:%v", err)
	}
	defer f.Close()
	...
	for {
		...
		if strings.HasPrefix(line, ">") {
			...
			geneId := strings.Split(hsplit[0], ">")[1]
			genbank := geneId + "|" + hsplit[1] + " " + strconv.Itoa(i)
			headerMap[geneId] = genbank
			i++
		}
		if err != nil {
			if err != io.EOF {
