...
//return a sequence channel
func buildConsensusGenome(snpChan chan *pileup.SNP, baseQual int8, fnaFile string) chan seq.Sequence {
	...
	//sequence := getSequence(baseChan, seqLens)
...
// openFile2 another helper
func openFile2(file *os.File) (f *os.File) {
	var err error
	f = file
	if err != nil {
		panic(err)
...
func writeFasta(seqChan chan seq.Sequence, file string, root string) {
	path := filepath.Join(root, file)
	f, err := createFile(path)
	if err != nil {
		log.Fatalf("Error when writing the following FASTA %s: %v", path, err)
	}
	defer f.Close()
...
// getSequence generates a sequence from the SNP chan.
func getSequence(baseChan chan Base, seqLens map[string]int) seq.Sequence {
	//seqChan := make(chan seq.Sequence)
